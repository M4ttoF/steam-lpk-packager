...
def process_item(item_id):
    ...
    if os.path.exists(dest_dir):
        print(f"  [CLEAN] Cleaning up pre-existing directory: {dest_dir}")
        shutil.rmtree(dest_dir)

    # 2. Copy the item to packages_lpk
    print(f"  [COPY] Copying {item_id} from Steam cache to packages_lpk...")
    start_time = time.time()
    try:
        shutil.copytree(src_dir, dest_dir)
        elapsed = time.time() - start_time
        print(f"  [OK] Copy completed in {elapsed:.2f}s.")
    ...
    if os.path.exists(dest_dir):
        print(f"  [CLEAN] Cleaning up temporary packages_lpk folder for {item_id}...")
        try:
            shutil.rmtree(dest_dir)
            print("  [OK] Temp folder deleted.")
        except Exception as e:
            print(f"  [ERROR] Failed to delete temp folder: {e}")
    ...
    if processed:
        ...
        print(f"[WARNING] Completed run for {item_id} but could not find a clear output package.")
        return False
...
def query_steam_metadata(ids):
    ...
    try:
        ...
        print(f"[ERROR] Error querying Steam API: {e}")
        return []
...
def run_dry_run(batch_file_path):
    ...
    if not items:
        print("[ERROR] No items found to query.")
        return

    ids = [item["id"] for item in items]
    annotations = {item["id"]: item["annotation"] for item in items}

    print(f"[API] Querying Steam Web API for {len(ids)} items...")
    details = query_steam_metadata(ids)
    if not details:
        print("[ERROR] Failed to retrieve metadata from Steam.")
        return
    ...
    print("\n" + "=" * 70)
    print(f"[SUMMARY]  STEAM WORKSHOP BATCH SUMMARY: {os.path.basename(batch_file_path)}")
    print("=" * 70)
    ...
    print(f"[ITEMS]  Total Workshop Items : {len(processed_items)}")
    if total_gb >= 1.0:
        print(f"[SIZE]   Total Download Size  : {total_gb:.2f} GB ({total_mb:,.1f} MB)")
    else:
        print(f"[SIZE]   Total Download Size  : {total_mb:.2f} MB")

    print(f"[LIVE2D] Live2D Models        : {live2d_count}")
    print(f"[SPINE]  Spine Models         : {spine_count}")
    print(f"[OTHER]  Other / Unclassified : {other_count}")

    # Disk space checking
    try:
        total_disk, used_disk, free_disk = shutil.disk_usage("/")
        free_gb = free_disk / (1024 * 1024 * 1024)
        print(f"[DISK]   Available Disk Space : {free_gb:.2f} GB")
        if free_disk < total_bytes:
            ...
            print("[WARNING] Disk space is tight! Download will fit, but buffer is less than 2x total size.")
        else:
            print("[OK] Safe to Download! You have plenty of free storage space.")
    except Exception:
        pass

    if largest_item:
        l_mb = largest_item["size"] / (1024 * 1024)
        print(f"[LARGEST] Largest Model        : '{largest_item['title']}' ({l_mb:.1f} MB) [ID: {largest_item['id']}]")
...
def main():
    ...
    if not to_process:
        print(f"[DONE] All models in {os.path.basename(batch_file)} are already fully processed!")
        return
    ...
    for index, fid in enumerate(to_process, 1):
        ...
        print("-" * 40)
        if not success:
            print(f"[WARNING] Item {fid} encountered issues, continuing with next...")

    print("\n" + "=" * 60)
    print(f"[DONE] ALL {os.path.basename(batch_file)} MODELS PROCESSED AND CLEANED!")
    print("=" * 60)
